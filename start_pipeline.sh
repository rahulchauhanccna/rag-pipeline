...
echo "=========================================="
echo ""
echo "Next steps (run these in separate terminals from the rag-pipeline directory):"
echo ""
echo "1. Start the order stream generator:"
echo "   cd rag-pipeline && $PYTHON_CMD data_generators/order_stream_generator.py --mode socket --self-connect"
echo ""
echo "2. Start the Flink job:"
echo "   cd rag-pipeline && $PYTHON_CMD flink-job/flink_job.py"
echo ""
echo "3. Run the RAG application (interactive mode):"
echo "   cd rag-pipeline && $PYTHON_CMD rag_app/rag_app.py --interactive"
echo ""
echo "Or test with a single query:"
echo "   cd rag-pipeline && $PYTHON_CMD rag_app/rag_app.py --customer-id 123 --query \"Why is my order delayed?\""
echo ""
echo "Service URLs:"
